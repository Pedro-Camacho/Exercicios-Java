    ...
    private void addFieldValidators() {
        ...
        txtName.setInputVerifier(new InputVerifier() {
            ...
            public boolean verify(JComponent input) {
                ...
                boolean valid = Validation.notBlank(field.getText());
        ...
        txtEmail.setInputVerifier(new InputVerifier() {
            ...
            public boolean verify(JComponent input) {
                JTextField field = (JTextField) input;
                boolean valid = Validation.isEmail(field.getText());
                highlightField(field, valid);
                return valid;
            }
        });
        ...
        txtBody.setInputVerifier(new InputVerifier() {
            @Override
            public boolean verify(JComponent input) {
                JTextArea area = (JTextArea) input;
                boolean valid = Validation.notBlank(area.getText());
                highlightField(area, valid);
                return valid;
            }
        });
    }

    // Método para mudar a borda do campo inválido
    private void highlightField(JComponent field, boolean valid) {
        if (valid) {
